...
event_type (drop_event, order_event, system_message)
...
(Each user can become a creator, but not all users are creators).

Creator ↔ Product → One-to-Many
(A creator can list multiple products).

Product ↔ Drop → One-to-Many
(Each product can have multiple scheduled drops).

User ↔ Order → One-to-Many
(A user can place many orders).

Drop ↔ Order → One-to-Many
(A drop can have many user orders).
